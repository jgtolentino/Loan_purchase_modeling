...
We drop all the columns (ID, Experience, and Zip) which we do not need for any manupulations i.e from which data we cannot make out any thing.

Here we should not consider the ID as its completely unique for each customer and does not help in model building

Age is highly correlated with Experience; we will consider Age to identify customers who will be
...
```{r}
bank.backup= bank #Creating a back up file before making data transformation for visualization, this data back up will be used later for scaling, which will then be use for data modeling
str(bank.backup)
```
...
plot_boxplot(bank, by = "Education", 
             geom_boxplot_args = list("outlier.color" = "red")) #Plotting boxplot by factor of Education for all the numerical variables
```
...
plot_boxplot(bank, by = "Personal.Loan", 
             geom_boxplot_args = list("outlier.color" = "blue")) #Plotting boxplot for Personal Loan (Response variable) for all numerical variables
```
...
Clustering
Primarily hierarchial and kmeans clustering are two best suited methods for unsupervised learning. Since we have a large dataset (5000 observations) we cannot use hierarchial method.Kmeans suits this type of data categorization.

Converting the data types the factor variables back to numeric which is the suitable data type for scaling. Converting multiple columns (Education, Personal.Loan, Securities.Account, CD.Account, Online, and CreditCard) into numeric columns.

```{r}
##  Converting multiple columns into factor columns
col1 = c("Education","Personal.Loan","Securities.Account", "CD.Account", "Online", "CreditCard")
bank[col1] = lapply(bank[col], as.integer)
str(bank)
```

```{r}
bank.scaled =scale(bank.backup) #Creating a scaled data using the back up data set which preserves the numeric data type which is suitable for data scaling and modeling
head(bank.scaled)
```
...
clusplot(bank.scaled,clust2$cluster,color = TRUE,shade = TRUE,
...
for(k in 1:5){
  set.seed(seed)
  ClustB=kmeans(x=bank, centers=k, nstart=5)
...
```{r}
set.seed(seed)
nc=NbClust(bank,min.nc = 2,max.nc = 5,method = "kmeans")
```
...
```{r}
set.seed(seed)
clustC=kmeans(bank.scaled,centers = 4,nstart = 5)
...
```
Output Analysis
Proportion of responders and non-responder in actual data set is 9.1% and 90.9% respectively.
Train data contains 3500 observation out of which proportion of responders is 9.1% and nonresponders is 90.9%.
Test data contains 1500 observation out of which proportion of responders is 10.7% and nonresponders is 89.3%.
...
```{r}
rcontrol=rpart.control(minsplit = 100,minbucket = 10,cp = 0,xval = 10) #Setting the control parameters
